...
const FP_ICON: Record<string, string> = {
  ...
  "Old account": "clock", 
  "Article": "file", 
  "Exposed contact info": "mail", 
  "Reputational risk": "flag",
};
...
export const FootprintScanner: React.FC<FootprintScannerProps> = ({
  ...
  const list = findings.filter(f => filter === "All" || f.type === filter);
  ...
  const handleToggleDismiss = (id: string) => {
    setDone(prev => {
      ...
      onToast(nextVal ? "Dismissed finding" : "Reopened finding");
  ...
  return (
    <div className="max-w-[1180px] mx-auto fade-in">
      <div className="flex justify-between items-end mb-4 flex-wrap gap-3">
        <div>
          <div className="text-[10px] tracking-[0.14em] uppercase text-t-2 font-semibold mb-1">Public Footprint Scanner</div>
          <h1 className="text-[26px] font-semibold tracking-tight text-t-0 leading-tight">Your public footprint</h1>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4.5">
        {[
          ["Findings", counts.Total, "globe", "t-1"],
          ["High visibility", counts.High, "eye", "crit"],
          ["Medium risk", counts.Medium, "alert", "med"],
          ["Low risk", counts.Low, "check-circle", "low"]
        ].map(([k, v, ic, c], i) => (
          <div key={i} className="border border-line rounded-lg p-4 bg-bg-2 shadow-premium">
            <div className="flex items-center gap-2 text-t-2 mb-2.5">
              <Icon name={ic as string} size={15} style={{ color: `var(--${c})` }} />
              <span className="text-[12px] font-semibold">{k}</span>
            </div>
            <div className="font-mono text-[26px] font-semibold text-t-0 leading-none">{v}</div>
          </div>
        ))}
      </div>
      ...
      {inlineAI && (
        <div className="mb-4">
          <AIInsightCard 
            tag="AI Footprint Analyst" 
            lead 
            confidence="High"
            body="The people-directory listing is your top removal — it ties your real name to a home address. I can draft the opt-out now." 
            action="Draft opt-out" 
      ...
      {/* Filter Tabs */}
      <div className="flex gap-2 mb-4.5 overflow-x-auto pb-1.5">
        {types.map(tp => (
          <button 
            key={tp} 
            className={`px-3 py-1.5 rounded-lg text-[12px] font-semibold border cursor-pointer transition-all duration-120 whitespace-nowrap ${
              filter === tp 
                ? "bg-bg-3 border-line-2 text-t-0" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {list.map(f => (
          <div 
            key={f.id} 
            className="border border-line rounded-lg p-5 bg-bg-2 shadow-premium flex flex-col justify-between transition-all duration-150"
          ...
          >
            <div>
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="w-[38px] h-[38px] rounded-lg bg-bg-inset border border-line-2 flex items-center justify-center text-teal flex-shrink-0">
                    <Icon name={FP_ICON[f.type] || "globe"} size={17} />
                  </div>
              ...
              <div className="flex gap-1.5 mb-3 flex-wrap">
                <span className="inline-flex items-center gap-1.5 text-[11.5px] font-semibold px-2 py-0.5 rounded-md bg-bg-3 border border-line text-t-1">{f.type}</span>
              ...
              <div className="text-[10px] tracking-[0.14em] uppercase text-t-2 font-semibold mb-1.5">Exposes</div>
              <div className="flex gap-1.5 mb-3 flex-wrap">
                {f.exposes.map(e => (
              ...
              <div className="text-t-3 text-[10px] mb-3.5 flex items-center gap-1.2 font-semibold">
                ...
                <span>AI analysis · Human review required before action</span>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-2">
              {f.type === "Data broker" || f.type === "Directory" ? (
                <button 
                  className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg font-semibold text-[12px] px-3.5 py-2 bg-gradient-to-b from-teal to-cyan text-[#04110F] hover:brightness-[1.07] active:translate-y-[0.5px] cursor-pointer transition-all duration-100"
                ...
                <button 
                  className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg font-semibold text-[12px] px-3.5 py-2 border border-line-2 bg-bg-3 hover:bg-bg-2 hover:border-line-3 text-t-0 cursor-pointer transition-all duration-130"
              ...
              <button 
                className="inline-flex items-center justify-center gap-1.5 rounded-lg font-semibold text-[12px] px-3 py-2 border border-line-2 bg-bg-3 hover:bg-bg-2 text-t-1 hover:text-t-0 cursor-pointer transition-all duration-130"
                onClick={() => handleToggleDismiss(f.id)}
                title={done[f.id] ? "Reopen finding" : "Dismiss finding"}
              >
